...
from .models import Post, Orders, Payment
import pyqrcode
from django.shortcuts import get_object_or_404
from django.http import JsonResponse
from .blockchain import RECEIVER_ADDRESS, get_transaction_data
import json
...
def payment_view(request, id):
    ...
    qr_image = qr.png_as_base64_str(scale=5)
...
def process_payment(request):
    if request.method == 'POST':
        ...
        try:
            tx_data = get_transaction_data(from_address, RECEIVER_ADDRESS, amount)
            return JsonResponse({
                'status': 'pending',
                'tx_data': tx_data,
                'order_id': id
...
def confirm_payment(request):
    if request.method == 'POST':
        ...
        logger.info(f"Received tx_hash: {tx_hash} for order_id: {order_id}")
